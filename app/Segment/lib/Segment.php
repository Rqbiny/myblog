...
class Segment {
	...
	function __construct($loaddic=true) {
  	if($loaddic) {
  	  for($i=0;$i<strlen($this->cn_one_name);$i++){
  		  $this->one_name_dic[$this->cn_one_name[$i].$this->cn_one_name[$i<strlen($this->cn_one_name)?$i:$i+1]] = 1;
		  $i++;
  	  }
  ...
  function get_keyword($str,$ilen=-1) {
    ...
    if(env('CHARSET', 'utf-8')=='utf-8') $okstr = iconv('gbk','utf-8',$okstr);
    return trim($okstr);
  }
  function get_rank($w){
